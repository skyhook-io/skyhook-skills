...
- **Cross-review the plan with the OTHER model — Claude, not Codex.** Hand the
  plan to Claude via the **claude-review** skill, or directly. Prefer writing the
  plan to a temp file and asking Claude to read it; that avoids argv quoting
  issues with backticks, `$`, newlines, and very large plans:
  ```bash
  # Plan text already written to /tmp/plan.md
  claude -p "Read /tmp/plan.md and critique it as a skeptical senior engineer. Find flawed assumptions, missing steps, hidden complexity, simpler alternatives, and risks. Challenge it; don't rewrite it. Return ranked findings only." --model opus --permission-mode default --allowedTools Read Grep Glob Bash --no-session-persistence --output-format stream-json --include-partial-messages --verbose
  ```
  ...
  can look hung during a long critique. Do **not** interrupt just because no
  final prose has appeared while `tool_use`, `status`, or `thinking_tokens`
  events are still arriving. Triage the final `result` text, not the JSON event
  noise.
